...
OUT    = Path('/tmp/boltbound-mock-cards-v2'); OUT.mkdir(exist_ok=True, parents=True)
...
def render_card(art_img, card):
    ...
    if eff:
        ...
        ey = panel_y0 + ((panel_y1 - panel_y0) - total_h) // 2
        ...
        # No text — fill panel with a flavor placeholder so no dead space.
        ef = font(20, 'body')
        placeholder = (', '.join(k.upper() for k in (card.get('keywords') or []))
                       or 'Vanilla minion.')
        lw = draw.textlength(placeholder, font=ef)
        draw.text(((eff_x0 + eff_x1) // 2 - lw // 2,
                   (panel_y0 + panel_y1) // 2 - 10),
                  placeholder, font=ef, fill=(180, 160, 220, 255))
