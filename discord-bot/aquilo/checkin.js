...
// own posts re-fired the handler. Check every known field name.
//
//   { guild_id, channel_id, message_id, content,
//     author: { id, username, global_name, bot },
...
export async function handleCheckinMessage(env, payload) {
  ...
  const userId = String(payload.user_id || payload.userId || payload.author?.id || '');
