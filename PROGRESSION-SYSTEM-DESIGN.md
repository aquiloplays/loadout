...
from the Twitch-panel `tw:<twId>` form, which the link table resolves, see §5.4).
...
engagement (first stream, first bolt, etc.). We **don't replace it**, we **extend it**:
...
- A **free track** and a **premium track**. Premium is unlocked
  for any active Patreon supporter (single-tier, all-or-nothing, same gate as the existing free-lootbox stipend; no per-tier
...
- Push notification on unlock (via existing Clash push gateway, rename it to "progression push" internally).
...
2. **Premium battle pass via Patreon, patron / non-patron, flat.**
   ...
   per-tier exclusive badge, no per-tier tournament-seed perk, premium is all-or-nothing. The existing `patreon:tier:<userId>`
